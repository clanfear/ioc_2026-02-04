...
cdc_mortality |>
  ...
  geom_text(data = ~ . |> 
              filter((type %in% c("All", "Firearm") & year == 1988)) |> 
              mutate(rate = ifelse(type == "Firearm", rate - 1, rate  + 1)), 
            aes(label = type), 
            family = plot_font,
            size = 6) +
...
## [Not attributable to age composition]{.r-fit-text}{background-image="img/pedro-lastra-5Bv3EcijAl4-unsplash.jpg" background-opacity="0.2"}
